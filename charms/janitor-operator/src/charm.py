...
ARCH_RELEASE_ALLOW_MAPPING = {"trusty": ["amd64", "i386"], "xenial": ["amd64", "i386"]}
...
class AutopkgtestJanitorCharm(ops.CharmBase):
    ...
    def __init__(self, framework: ops.Framework):
        ...
        self._stored.set_default(workers=set())
    ...
    def _on_config_changed(self, event: ops.ConfigChangedEvent):
        if self.releases != self.typed_config.releases:
            ...
            new_releases = [
                release for release in new_config if release not in current_config
            ]
            old_releases = [
                release for release in current_config if release not in new_config
            ]
