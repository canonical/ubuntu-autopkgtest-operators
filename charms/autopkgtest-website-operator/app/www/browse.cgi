...
def get_results(limit: int, offset: int = 0, **kwargs) -> list:
    ...
    requested_result = exitcode_from_string(kwargs.get("result", None))
    ...
    if db_has_result_requester_idx(cursor):
        ...
        # special handling for result, since it may be multiple exit codes
        if requested_result:
            codes = ",".join(str(code) for code in requested_result)
...
def recent():
    ...
    recent_test_results = get_results(
        limit, offset, arch=arch, release=release, user=user, result=result
    )
    ...
    if flask.request.path.endswith(".json"):
        ...
        all_results = [k for k in EXITCODES.keys() if k != "otherfail"]
