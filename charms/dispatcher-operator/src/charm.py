...
AUTOPKGTEST_PACKAGE_CONFIG_LOCATION = pathlib.Path(f"~{USER}/autopkgtest-package-configs").expanduser()
...
SYSTEMD_UNIT_FILES_PATH = "/var/lib/juju/agents/unit-autopkgtest-dispatcher-*/charm/units"
...
class AutopkgtestDispatcherCharm(ops.CharmBase):
    ...
    def __init__(self, framework: ops.Framework):
        ...
        self.typed_config = self.load_config(config_types.DispatcherConfig, errors='blocked')
        ...
        framework.observe(self.on.show_target_config_action, self._on_show_target_config)
        framework.observe(self.on.create_worker_units_action, self._on_create_worker_units)
    ...
    def clone_repositories(self) -> None:
        for (repo, branch, location) in [(AUTOPKGTEST_REPO, AUTOPKGTEST_BRANCH, AUTOPKGTEST_LOCATION),
                                        (AUTOPKGTEST_CLOUD_REPO, AUTOPKGTEST_CLOUD_BRANCH, AUTOPKGTEST_CLOUD_LOCATION),
                                        (AUTOPKGTEST_PACKAGE_CONFIG_REPO, AUTOPKGTEST_PACKAGE_CONFIG_BRANCH, AUTOPKGTEST_PACKAGE_CONFIG_LOCATION)]:
    ...
    def _on_set_unit_count(self, event: ops.ActionEvent):
        ...
        event.set_results({"results": f"Set unit count for {worker_arch} to {params.count}"})
    ...
    def _on_create_worker_units(self, event: ops.ActionEvent):
        self.systemd_helper.set_up_systemd_units(self._stored.workers, self.typed_config.releases.split(' '))
    ...
    def _on_amqp_relation_joined(self, event: ops.RelationJoinedEvent):
        self.unit.status = ops.MaintenanceStatus(f"Setting up {event.relation.name} connection")
